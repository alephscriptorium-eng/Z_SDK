 ...
 * `caudal:<view>:<id>` — la ley del proyecto: la operativa vive en HTML
...
/** Clave de persistencia por vista: `caudal:<view>:<id>`. */
export function panelStorageKey(view, id) {
  return `caudal:${view || 'view'}:${id}`;
}
 ...
 * @param {string}  [opts.view]           vista actual (clave `caudal:<view>:<id>`)
