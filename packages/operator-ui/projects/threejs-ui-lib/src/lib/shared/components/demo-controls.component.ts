...
export interface DemoControlState {
  ...
  /** Live session: deck B has a resolved registro target for cast. */
  castEnabled?: boolean;
  /** Live zeus — Send One casts without requiring demo Start. */
  ...
  template: `
    <div class="demo-controls">
      ...
      <div class="button-group">
        ...
        <button (click)="onSingleMessage()" [disabled]="!canSendOne()" title="{{ sendOneTitle() }}">📤 Send One</button>
...
export class DemoControlsComponent {
  ...
  canSendOne(): boolean {
    if (this.state.liveSessionMode) {
      return this.state.castEnabled === true;
    }
    return this.state.isRunning;
  }

  sendOneTitle(): string {
    if (this.state.liveSessionMode && !this.state.castEnabled) {
      return 'Cue deck B (lineas) y espera registros resueltos';
    }
