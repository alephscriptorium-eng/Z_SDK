 ...
 */
export function buildTypeDeclarations() {
  const roleUnion = ROLES.map((r) => `'${r}'`).join(' | ');
  const kindUnion = EVENT_KINDS.map((k) => `'${k}'`).join(' | ');
  const eventConsts = Object.entries(EVENTS)
    .map(([k, v]) => `  readonly ${k}: '${v}';`)
    .join('\n');
  const gateConsts = Object.entries(GATES)
    .map(([k, v]) => `  readonly ${k}: '${v}';`)
    .join('\n');
...
export type EventKind = ${kindUnion};
...
export declare const SNAPSHOT_BUDGET_BYTES: ${SNAPSHOT_BUDGET_BYTES};
