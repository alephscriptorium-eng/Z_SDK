/**
 * Gate exports ↔ declarations for @zeus/acta-kit (U246).
    ...
    const walk = (dir) => {
      for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (ent.isDirectory()) walk(abs);
          ...
          declarations += 1;
          const rel = path.relative(typesDir, abs).replaceAll('\\', '/');
          if (!declaredFiles.has(path.resolve(abs))) {
            // May still be reachable via barrel import — check index/barriles.
            const reachable = isReachableFrom(declaredFiles, abs, typesDir);
            if (!reachable) {
              add('B', 'orphan_declaration', rel);
            }
          }
        }
      }
...
if (import.meta.url === `file://${process.argv[1].replaceAll('\\', '/')}` ||
  ...
  console.log(
    result.ok
      ? `PASS acta-kit gate — ${result.checked.subpaths} subpaths, ${result.checked.declarations} declarations`
      : `FAIL acta-kit gate — ${result.findings.length} finding(s)`
  );
