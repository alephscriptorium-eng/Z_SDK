...
/**
 * @zeus/player-debug-3d-ui server.
 *
 * Same scaffold as player-3d-ui (Express, server-rendered, no bundler) — the
 * difference is the browser scene (radial room-traffic, not the map engine).
 * Serves:
...
import { debugView } from './views/main_views.mjs';
 ...
 */
export async function createDebug3dServer(options = {}) {
  ...
  try {
    ...
    console.warn(`[player-debug-3d-ui] three not resolvable, /vendor/three disabled: ${err.message}`);
  }
  ...
  app.get('/', (req, res) => {
    try {
      ...
      console.error('Error rendering debug 3D viewer:', err);
      res.status(500).send(`<h1>Error</h1><p>${err.message}</p>`);
    }
  });

  app.use((err, _req, res, _next) => {
    console.error('player-debug-3d-ui error:', err);
...
if (isMain) {
  const handle = await createDebug3dServer();
  console.log(`player-debug-3d-ui running on http://${handle.host}:${handle.port}`);
