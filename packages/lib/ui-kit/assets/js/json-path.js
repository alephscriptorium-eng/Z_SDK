/* GENERATED — do not edit. Run: npm run build:json-path -w @zeus/ui-kit */
(function (global) {
  'use strict';
 ...
 */
function isRootPath(path, rootLabel = DEFAULT_ROOT) {
 ...
 * @returns {string[]}
 */
function parsePath(path, rootLabel = DEFAULT_ROOT) {
 ...
 * @returns {string}
 */
function formatPath(segments, rootLabel = DEFAULT_ROOT) {
 ...
 * @returns {unknown}
 */
function getAtPath(root, path, rootLabel = DEFAULT_ROOT) {
 ...
 * @returns {string}
 */
function getParentPath(path, rootLabel = DEFAULT_ROOT) {
  const segments = Array.isArray(path) ? [...path] : parsePath(path, rootLabel);
 ...
 * @returns {string}
 */
function previewValue(value, maxLen = 80) {
  if (value === null) return 'null';
 ...
 */
function getSiblingPaths(path, root, rootLabel = DEFAULT_ROOT) {
  const segments = parsePath(path, rootLabel);
 ...
 * @param {string} [rootLabel]
 */
function inspectAtPath(root, path = DEFAULT_ROOT, rootLabel = DEFAULT_ROOT) {
 ...
 */
function buildFocusExport(root, path = DEFAULT_ROOT, opts = {}) {
  const rootLabel = opts.rootLabel ?? DEFAULT_ROOT;
