...
fichero** (existencia + sha256 + tamaño) antes y después de ejecutar. Ancla la
operación; la notación le es indiferente por construcción.
...
léxico (contención) → nombre de artefacto → **identidad física del inodo** →
extensión.
...
de **otro** root, escape por `..`, el root mismo, extensión no-JSONL, enlace
duro al manifiesto, junction de directorio que salta fuera; ausencia en siete
formas; `importPack` con `ledgerPath` envenenado (manifiesto intacto y sin
JSONL dentro).
...
- **La sonda es DINÁMICA: sólo ve lo que se ejecuta.** No sustituye al censo
  ...
  fuera de mi ALCANCE_DIFF. El cambio que lo cerraría es de una línea, sobre
  `export.test.mjs:397`: añadir `|resolveManifestPath` a `TOKEN_DE_MANIFIESTO`.
  Queda **propuesto, no hecho**, y no es suficiente por sí solo (la notación C,
  por composición de cadena, seguiría invisible).
- **Reescritura de bytes idénticos: invisible** para la sonda. Se intentó cerrar
  con `mtime` y **se descartó por medición**: cambió en 19 de 30 reescrituras
  idénticas en win32, es decir, sería intermitente. Nótese que el sello ES el
  sha256 de los bytes, así que una escritura que preserva los bytes preserva el
  sello — pero viola la lectura-sólo, y eso queda sin cubrir.
...
- **Una sonda por monkey-patch de `fs` NO habría servido.** Medido: parchear
  ...
  ```
  Consecuencia para el carril: el probe dinámico CA-5a de U205 funciona **hoy**
  sólo porque `export.mjs:114` hace `import fs from 'node:fs'`; el día que ese
  fichero pase a importaciones nombradas, CA-5a se queda ciego **en silencio**.
  Observación reportada, **no arreglada** (fuera de alcance).
...
- **`ledgerPath` deja de admitir rutas arbitrarias**: ahora debe ser `.jsonl` y
  ...
  el manifiesto». No rompe a nadie hoy: `grep -rn "ledgerPath" packages e2e
  scripts` no encuentra ningún llamante que la pase, salvo la relectura interna.
  Si mañana alguien quiere el ledger fuera del root, tendrá que discutirlo.
...
# tests 207   # pass 205   # fail 0   # skipped 2      (antes: 194 / 192 / 0 / 2)
