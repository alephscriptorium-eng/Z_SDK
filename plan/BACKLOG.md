...
| **U266** ✅ | — | **El catálogo puede anunciar un puerto donde no hay nadie escuchando** (hallado por la contrarrevisión de U181; **enrutado a `U227`, que está CERRADO** — ficha propia, 2026-08-02). `presets-sdk/src/env/index.mjs:169-175` (`Number(raw)` + `Number.isFinite`) acepta `0`, `-1`, `65536`, `3.5`, `0x10`→16, `"  "`→0 y `"03012"`→3012. MEDIDO punta a punta con `ZEUS_PORT_EDITOR=0`: **el catálogo anuncia `0` y el bind real cae en un puerto efímero** (56206/59282 según la corrida), health 200 en el real y `fetch failed` en el anunciado. Es exactamente el defecto que U181 vino a cerrar, entrando por la puerta del valor mal formado. **Atenuante**: falla ruidosamente, no da falso verde. **Y dos hechos que hoy no están declarados en ningún sitio**: con `.env` y variable de proceso a la vez **gana el proceso** (dotenv sin `override`, medido: `.env=14012` + proceso `15012` → 15012), y **ceros a la izquierda y espacios cuelan**  ✅ **ACEPTADO 2026-08-03** (obra `fbcfe24`, B1 `ef9d0ad`, B2 `b9d97b2`, B3+B4 `0f3336a`, menores `5446cc4`, merge `626cb8d`; **CI verde en el tip exacto `5446cc4` Y en el commit de merge**; **4 contrarrevisiones adversariales**: 1 bloqueante → 1 → 2 → **cero**). **EL CENSO DEL DEFECTO CRECIÓ 1 → 3 → 4 → 5 FICHEROS DE SERVIDOR, Y EL CRECIMIENTO ES EL DATO** (va en tabla en el reporte): cada vuelta amplió el instrumento y **cada ampliación encontró uno más**. **LA LECCIÓN DEL WP, escrita con esas palabras**: *«corregir el encuadre de un censo no es corregir su instrumento»* — aceptó en la 2.ª vuelta que el criterio estaba mal («quién llama a `readEnvPort`» → «quién convierte una cadena en un puerto de escucha») **y rehízo el censo con el mismo `grep` anclado en `process.env.`**, que por construcción **no puede ver una lectura por parámetro**; el residuo del instrumento viejo era exactamente el fichero que faltaba. **LA SONDA QUE POR FIN LO CERRÓ NO BUSCA FORMAS SINTÁCTICAS** —lista inagotable— **sino que compara las claves con pinta de puerto contra las 33 que declara la fuente única**: exactamente 3 alias, ningún sexto fichero, y el revisor lo confirmó con su propio censo (**24** funciones con mapa de entorno por parámetro, **cero** leyendo claves de puerto). El precedente **ya estaba escrito en `scripts/gates/claves.mjs:718-723`**. **SU HALLAZGO ESTRELLA, REPRODUCIDO POR EL REVISOR CON OTRO MECANISMO** (`module.registerHooks`, sin tocar el árbol): **una guarda SÓLO en `readEnvPort` habría EMPEORADO el defecto** — con `ZEUS_MCP_SUN=0` el catálogo daba `solar-sun port = 4101` con **rc=0**, o sea un arranque verde en el puerto por defecto; el fallo ruidoso se habría convertido en uno invisible **justo al «arreglarlo»**. Por eso la guarda va en **dos** sitios y dos `catch` mudos dejan de tragarse el error de configuración. **B1: el `.env.example` prometía lo que no cumplía.** El WP añadió una cabecera diciendo que un valor mal formado aborta, y era **falsa para dos claves del propio fichero** (`socket-server` y `operator-ui` arrancaban en verde con `0` → efímero, y con `03012` → **3012, colisionando con editor-ui**). *Un silencio se descubre; una promesa escrita hace que nadie mire.* Cerrado **por arriba**, no acotando la promesa. **B2: el quinto fichero daba el defecto ENTERO SIN NINGÚN VALOR MAL FORMADO** — `game-bridge.mjs` y `serve.mjs`, mismo paquete, leían los dos mismos nombres **en orden contrario**: con `ZEUS_PORT_WEBRTC_VIEWER=4001 WEBRTC_VIEWER_PORT=4002`, uno ataba 4002 y el otro anunciaba 4001. **Cerrado haciendo la divergencia inexpresable**: el orden **deja de estar escrito en ningún servidor** y se pide a `uiPortEnvChain()` en la fuente única. **B4: el guardián de «anunciar == atar» calculaba las DOS MITADES DEL MISMO SITIO**, así que no podía discrepar nunca — con la lista invertida a mano en `serve.mjs` la suite quedaba **25/25 verde con B2 reintroducido**. Hoy **arranca el servidor en proceso hijo y lee `server.address().port`**, y con eso `serve.mjs` **estrena tests que muerden** (antes no tenía ninguno: deshacer entero su arreglo dejaba 25/25 verde). **REGLA NUEVA QUE DEJA, y vale para todo el programa: un guardián ancho enmascara el cableado de cada fichero.** Pasó **cuatro** veces con `resolveZeusUiPorts` — un test verde **al ablacionar su propio guardián** porque lo sostenía otro; *no es que el rojo venga de otro, es que el VERDE lo sostiene otro*. **Y la cuarta la creó su propio arreglo**: al ensanchar el guardián para cerrar un menor, **retro-enmascaró dos tests suyos** y dejó vivo un comentario que decía «éste es el caso que nadie más cubre», falso desde ese momento. **De ahí: las ablaciones se corren DESPUÉS del último cambio, no antes.** **SU PROPIA GUARDA TUMBÓ EL GATE `GD`** (`e2e/local-first-ca.mjs` ponía `ZEUS_MCP_FORCES='0'` a propósito) y **NO la relajó**: cambió el arnés a pedir puerto libre al SO, y de ahí sale un contrato más fino que el de la ficha — **`{port:0}` en código es idioma vivo; `ZEUS_*=0` desde el entorno aborta**. `GD` queda **7/7 · 14 vectores rojos · RC 0**. **DOS FALSOS VERDES CAZADOS POR ÉL Y UNO POR EL REVISOR, todos del mismo tipo**: un arnés de ablación cuyo patrón **no encajaba por CRLF** daba verde sin aplicar nada — *«un arnés de ablación que no encaja es un falso negativo mudo»*; el revisor **lo reprodujo sin querer, mismo árbol y misma causa**. **OCTAVO VECTOR, descubierto por él y no en la ficha**: `ZEUS_PORT_SCRIPTORIUM=''` daba `Number('')` = **0**, o sea que **una clave vacía ataba un efímero**. **M-1, y es la clase del WP en una promesa de API**: el docstring decía *«quien pasa su propio mapa está diciendo resuelve contra ESTO»* y **no es cierto en la dirección del fallo** — un `process.env` mal formado tumba al llamante aunque pase un mapa propio y válido. No se cambió la conducta (abortar es lo que pide la CA y falla ruidoso); se corrigió la promesa **en el JSDoc, en el `.d.ts` publicado y con un ASERTO**, para que no vuelva a separarse del código. **M-2: el ÚNICO test de bind que corría en CI no fijaba el mecanismo que decía fijar** — con `readEnvPort` leniente, `app-shell` quedaba 15/15 verde porque abortaba `readEnvPortAlias` de rebote. Reencaminado a un camino donde **el único validador posible es `resolveAppPort`**, verificado **por pila de llamadas**, y la ablación re-corrida **después**: `8 · 7 pass · 1 fail`, «nadie disparó». **Límites declarados (20 en §7)**: `mcp-launcher`, `webrtc-viewer` y `oasis-webrtc` **fuera de la matriz de CI** — cuatro de ocho ficheros de test sólo corren en local; `webrtc-viewer/test/puerto-mal-formado.test.mjs` **no muerde ninguna ablación** y lo dice en su cabecera; **un intermitente de `mcp-launcher` observado y NO explicado, 1 de 30** (1/10 suyo + 0/20 del revisor), declarado con denominador en vez de callado; `ZEUS_DISCOVERY_URLS` mete puertos sin validar en la lista de discovery (severidad baja, **el límite de la sonda hecho caso con nombre**); el orden de alias **no es uniforme a propósito** (1 con canónica primero, 2 con alias primero — uniformarlo movería el puerto a quien ya lo tenga configurado), declarado como deuda con nombre; la garantía es sobre **configuración**, no sobre todo puerto posible (`{port:-1}` explícito desde código sigue pasando, y Node lo rechaza con `ERR_SOCKET_BAD_PORT`) | un puerto mal formado **falla al arrancar**, no se anuncia · la precedencia `.env` vs proceso **escrita** donde se lea · vectores de los siete valores medidos | P1 |
...
| **U271** ⬜ | — | **Mecanizar la invariante del lexer: que una rama que consuma entrada y calle enrojezca SOLA** (diseño propuesto por la contrarrevisión de `U269` en su 6.ª vuelta, y aceptado por el orquestador **como ficha propia y no como séptima vuelta**, porque `U269` ya entrega una superficie enumerada y correcta y mecanizar la enumeración es trabajo con nombre). **Por qué**: `U269` cerró cuatro apariciones de la misma clase —una salida que consume entrada y no emite, no marca opaco y no lanza— y las cerró **con la invariante escrita**; pero su autor lo declara sin adornos: *«he cambiado una lista de formas por una lista de ramas, y la segunda es finita y está enumerada, pero **sigue siendo una lectura mía**»*. **Una lista de formas es infinita** —lo demostraron cuatro devoluciones seguidas— **y una lista de ramas es finita y auditable**, pero mientras la sostenga un par de ojos, la rama número diez que alguien añada mañana no la ve nadie. **El diseño, que es barato**: un **contador de cobertura de posición** en modo test — que `camposDeCodigo` lleve un array de intervalos `[ini, fin)` marcados por cada rama al consumir (las que emiten campo u opaco marcan **cubierto**; las tres que declaran no-dato —salto, blanco, puntuación— marcan **declarado**), y que un test exija que **la unión sea `[0, n)` sin huecos**. Una rama nueva que consuma y calle **deja un hueco y enrojece sola**. **Es la ley 1 de `U269` aplicada a código por POSICIÓN en vez de por token**, y resuelve exactamente lo que allí no podía: un token suelto en código es un identificador legítimo, pero **un rango de bytes ni consumido-y-emitido ni declarado es siempre un agujero**. Ficha abierta por el orquestador el 2026-08-03 | la unión de intervalos cubre el fichero entero, sin huecos, sobre el corpus real · **una rama nueva que consuma y calle enrojece sin que nadie la lea** (con mutante que la añada y control que la quite) · las tres ramas «declarado no-dato» nombradas una a una, no como clase · el instrumento **se puede matar y su muerte enrojece** (`L-H12` §5) · coste medido antes y después | P1 |
| **U270** ⬜ | — | **La suite de `@zeus/player-3d-ui` NO TERMINA en esta máquina, y termina en CI** (hallado por el orquestador el 2026-08-03 al podar la sala de `U266`: la retirada del worktree falló con «Directory not empty», y debajo había **tres procesos node vivos desde hacía 88 minutos** — `npm test -w @zeus/player-3d-ui` → `node --test test/*.test.mjs` → `node test/server.test.mjs`, encadenados padre-hijo-nieto, **con `packages/mesh/player-3d-ui` tomado como directorio de trabajo**). **Medido**: **cero puertos en escucha** de esos tres PIDs, o sea que no está sirviendo, **está colgada**; el paquete **SÍ está en la matriz de CI** (`ci.yml:78`) y **CI salió verde** en `5446cc4` y en el merge `626cb8d`. Así que la suite **termina en el contenedor limpio y no termina aquí** — el modo de fallo depende del entorno, que es justo el que nadie ve. **Por qué importa más que su tamaño**: (a) es un **falso verde de tiempo** —quien lanza `npm test` en local no obtiene ni verde ni rojo, obtiene *nada*, y lo natural es interpretar el silencio como «va lento»—; (b) **tapó la poda de la sala**: Node reintenta borrar un fichero abierto pero **no reintenta `rmdir` de un directorio que otro proceso tiene como cwd**, así que el resto quedó anclado y sólo se vio porque la orden de retirada falló; (c) es la **misma clase que el «siguiente invisible» de `HUB-110`** —procesos vivos al cierre de una corrida que se declara terminada— aparecida en nuestro propio árbol y **sin que ningún gate la mirara**. **Sospecha a verificar, no afirmación**: un asa abierta (servidor, `setInterval`, `watcher`) que impide drenar el bucle de eventos; en CI el contenedor muere igual y el fallo no se ve. **Cerrado por el orquestador matando los tres de dentro afuera y comprobando `node` a cero antes de borrar** | reproducido con nombre: **qué asa** queda abierta y en qué test · la suite **termina o falla**, nunca cuelga (con `--test-force-exit` sólo si se declara por qué, o cerrando el asa, que es lo bueno) · **un guardián de tiempo**: una suite que excede su presupuesto **enrojece**, en local y en CI · censo de las demás suites del monorepo por la misma prueba — porque si ésta cuelga, la pregunta es cuántas más | P1 |
| **U269** ✅ | — | **El detector de claves necesita parseo real, no mas regex** (acotado por la contrarrevision de U231, que lo separa a proposito para que U231 no entre en carrera armamentistica). Tres formas corrientes del corpus **pasan** hoy, medidas: un valor dentro de un **array JSON** (`{"tokens": ["…"]}`) porque la comilla corta la clase del valor · un **YAML block scalar** (`api_key: |` con el valor en la linea siguiente) — y `VOLUMES/DISK_02/LINEAS/registry.yaml` **es YAML real** · y `ENV API_KEY valor` **sin `=`**, que esta dentro del corpus propio de `contexto-imagen`. **Por que va aparte**: el detector es barrido de texto crudo, y cubrir estas formas con mas expresiones regulares empeora la densidad de falsos positivos, que ya es **90 hallazgos en 1741 ficheros trackeados, todos falsos**. La via es **parsear JSON y YAML de verdad** en los formatos que lo admiten, y dejar el barrido crudo para el resto  ✅ **ACEPTADO 2026-08-03** (9 commits, `fbcf…`→`3a1d326`, merge `2c167dc`; **CI verde en el tip exacto `3a1d326` Y en el commit de merge**; **6 contrarrevisiones adversariales**: 2 bloqueantes → 2 → 2 → 1 → 1 → **cero**. La ficha más peleada del programa). **LO QUE SE ENTREGA**: falsos positivos **145 → 98 sobre 1763** ficheros, coste **2151 ms contra 3042** del detector que sustituye, **cero pérdidas frente a él** y **cero dependencias nuevas** — el gate corre en un worktree **sin `node_modules`**, que es la razón por la que U231 lo construyó así. Analizadores propios en `scripts/gates/formatos.mjs`: JSON descendente-recursivo con posición, JSONL, subconjunto de bloque YAML, instrucciones de Dockerfile y un lexer de código. **LO QUE COSTÓ SEIS VUELTAS NO FUE EL RIGOR, SINO LA CLASE DE OBJETO QUE SE ENUMERA** —palabras suyas—. **Cuatro devoluciones seguidas fueron LA MISMA CLASE DE AGUJERO con distinta sintaxis**: algo que consume entrada y **no emite, no marca opaco y no lanza**. `flujoYaml` devolvía `null` (B1) · un `catch` se tragaba la duda y el literal de cadena no tenía suelo (B3) · un `continue` descartaba el comentario dentro de una continuación de Dockerfile (B5) · la rama de expresión regular consumía el literal entero (B7). **Cada vuelta trajo un instrumento más ambicioso y por cada uno entró el siguiente**, porque los tres primeros eran listas: de nombres, de formas sintácticas, de canarios. **LA QUINTA VUELTA SALIÓ DEL BUCLE CON UNA INVARIANTE**: *toda rama que consume entrada, o emite campo, o la marca opaca, o lanza; ninguna consume y calla, salvo las que **declaran con su nombre** que lo que consumen no es dato*. **Y la prueba de que el salto es real la dio el revisor: contó las ramas por su cuenta, leyendo el bucle ANTES de mirar la tabla, y coincidieron.** *Una lista de formas es infinita; una lista de ramas es finita y se audita en una tarde.* **LA LEY DE CONSERVACIÓN, que es el corazón del instrumento** (`test/gates/conservacion.mjs`): *todo token de la entrada tiene que aparecer en el nombre o el valor de algún campo*, más *ninguna campo no opaca lleva dentro una forma `nombre: valor` con material sin que una opaca la cubra*. **Encontró sola tres huecos que nadie buscaba** (clave que abre bloque, clave de objeto JSON, clave con colección de flujo — seis ficheros de `spec/`). Y su límite, declarado por el autor: **demuestra que no se pierde entrada, no que se lea bien** — el orquestador puso ese ataque el primero y **el revisor confirmó que esa puerta no da a ninguna pérdida** frente a `main`. **LA LECCIÓN MÁS TERCA, y va a `L-H12`: EL GUARDIÁN QUE VIGILA NO ESTABA VIGILADO.** Tres instrumentos seguidos del mismo autor —censo de mutación, recuento sintáctico y la propia ley— **apagables por completo con la suite en verde**: la ley se sustituía por `return []` y **las 96 pruebas seguían pasando**, porque su control positivo **simulaba** la pérdida con datos a mano en vez de **llamar** a la función. Su diagnóstico: *«no aplicaba a MIS instrumentos la doctrina que aplicaba al código — ninguno era matable, y por tanto ninguno vigilaba»*. Cerrado: apagar la ley entera → 3 rojos, ley 1 → 2, ley 2 → 1, **y el CONSUMO del resultado también vigilado**. **NO COMPRA SILENCIO CON RUIDO**: los falsos positivos **subieron** al cerrar agujeros (61→83→88→93→98), y la decisión se sostuvo con dato y no con postura — **cada hallazgo nuevo cae en una línea que el detector anterior TAMBIÉN señalaba** (23/23, 12/12, 11/11, medido con `comm`). *Es ruido que la versión anterior compraba con silencio, y en un guardián de credenciales ése es el intercambio equivocado.* **C1, cerrada por una medición que partió la decisión en dos**: extender la ligadura para saltar el **signo unario** recupera 3 de 4 formas **a coste cero**; saltar además el **`[` de apertura** recupera la cuarta pero **cuesta 1 falso positivo real y medido** (`const SEMILLAS = […]` en `arbol-inmutable.test.mjs`, porque `semillas` es léxico de identidad en castellano). **Cierra el signo, no cierra el corchete**, y la cuarta queda declarada con las de su clase. **CORRIGIÓ AL ORQUESTADOR TRES VECES CON DATO**: la cifra del brief (90/1741) **no se sostenía** —el árbol tenía 1759 ficheros y el detector daba 125—; **`LEXICO_ANCLADO` no anclaba**, bug preexistente de U231 (alternancia de primer nivel sin paréntesis: `author`, `tokenizer`, `secretaria` daban verdadero) **y los falsos positivos que producía eran del camino nuevo, no deuda de U231** (main daba 0 líneas con `author`); y las cifras del léxico **habían caducado dos veces**. **EL ÚLTIMO ROJO FUE DE CI Y ENSEÑÓ LA TRAMPA SIMÉTRICA**: reportó `lint RC 0` y CI enrojeció con dos `no-useless-escape` suyos — **`npm run lint` salía a 0 sin ejecutar nada** porque `eslint` no está en esta máquina. *Un `rc` de una orden que no existe no es una medición: es un silencio con forma de éxito.* Cerrado **con control positivo que reproduce las dos posiciones exactas que reportó CI**, y declarando qué no cubre esa medición. Su lección: *«la asimetría con la que trataba los rojos y los verdes — un rojo lo investigaba, un verde lo daba por bueno»*. **206 tests · 44 mutantes / 0 vivos / 0 no aplicados · ley 0 violaciones · canario 1006 ficheros con clave inyectada al principio y al final, 0 tragados · blobs en LF sin bytes de control** (entregó dos veces un byte de control invisible —un NUL y un `\x01`— que volvían el fichero binario y desactivaban la normalización, convirtiendo un diff de 72 líneas en uno de 2819). **Límite serio que queda declarado, y ya tiene ficha (`U271`)**: *«he cambiado una lista de formas por una lista de ramas, y la segunda es finita y está enumerada, pero **sigue siendo una lectura mía**»* — la mecanización por cobertura de posición la propuso la contrarrevisión y va aparte. Siguen abiertos: Markdown, `.env` y texto plano en barrido crudo (22 de los 98 son prosa de `.md`), frase de paso con espacios literales, `AUTHTOKEN` sin separador, ficheros >1 MiB y el subconjunto de YAML | las tres formas cazadas con parseo, no con regex · los 90 falsos positivos medidos **bajan**, y el numero se declara antes y despues · los siete falsos positivos censados en la contrarrevision de U231 como casos de contraprueba | P1 |
