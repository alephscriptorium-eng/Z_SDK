...
| **U234-B1** ⏳ | — | **Defecto vivo en el orquestador de U234** (hallado por U180, reproducido sobre la base sin sus cambios): `orchestrator.mjs:228` usa `netstat -ano -p tcp` (**solo IPv4**) y `socket-server` escucha en `[::1]:3017` → `status` declara `listening:false` con el servicio vivo, y `stop` sale EXIT 1 con `residualPids:[]` (salida contradictoria) **dejando el proceso huérfano**. La CA «stop sin residuos» de U234 se demostró sobre fixture IPv4, que es justo el caso que no falla. El escape del operador sí funciona (`npm run stop:services`, `stop-ports.sh:7`, sin filtro). Corrección de una línea | `stop` mata también el listener IPv6 · CA re-probada sobre `[::1]` | **P0** |
| **U237-B3** ⏳ | — | **La licencia no viajó al lock** (hallado por U197, **confirmado con números por su contrarrevisión: 50 entradas `"license":"AIPLv1"` en el lock frente a 48 manifests de paquete + la raíz** diciendo otra cosa): `package-lock.json` sigue declarando `"license":"AIPLv1"` mientras los 52 `package.json` dicen `SEE LICENSE IN LICENSE.md` (decisión ② del custodio, ejecutada en U237). El lock se cerró sin regenerar, así que **un gate de licencia que lea el lock leería el valor viejo** — y el lock es lo que viaja a una instalación reproducible. Cara Z; el patrón vale para L·O·G·HUB | lock regenerado y coherente con los 52 manifests · gate que lea **ambos** y falle ante divergencia | **P0** |
| **U202-B2** ⏳ | — | **`npm run gates` en rojo desde antes de la ola 3** (hallado por U180): 3 offenders `two-games` en `linea-kit/src/curation.mjs` y `volumes-ops/src/driver-lineas.mjs`, introducidos por `ca698d0`/**U202**; arrastra 1 de 23 en `test:gates`. Territorio del carril D → lo cierra el worker del carril, no una ola | `npm run gates` verde · offenders 0 | P1 |
