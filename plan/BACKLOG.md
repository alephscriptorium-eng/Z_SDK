...
| **U204** 🔶 | 35 | Driver FIREHOSE | es flujo: definir **unidad** (cursor/clave) antes de tocar transporte; unión, nunca sobrescritura. **Despachado 2026-07-31 al deshielo** (rama `wp/u204-driver-firehose`, worktree `C:\S_LAB\wt\z-u204`, base `26d1470`): brief = los riesgos que U203 le cita (unión aditiva por clave = Z-D9 · **matar o reencaminar el escritor legado `feed-kit/src/jetstream-sync.mjs:119` cuyo `syncedAt` rompe el hash** · detect sin firma en disco · sync vivo = estado, manifiesto solo por import) | import incremental idempotente sobre los 8.388 del censo | P1 |
